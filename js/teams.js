...
import { MARGIN, FIELD } from "./config.js?v=b10";
import { NATION_TEAMS, NATION_RATINGS, NAME_POOLS, NATION_STARS, NATION_OVERRIDES } from "./nations.js?v=b10";
...
const SQUAD_OVERRIDES = {
  ...
  bav: {
    ...
    5: { name: "Jacob Jajo", number: 6, speed: 1.3 }, // DM — bleibt, aber schneller
...
export function buildSquad(team, attackRight) {
  ...
  const squad = formation.map((slot, i) => {
    ...
    return {
      ...
      speed: ov.speed,   // optionaler Tempo-Faktor (z. B. schneller Spieler)
      homeX,
      homeY,
    };
  });
